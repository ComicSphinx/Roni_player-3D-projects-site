## Languages
1. python 3.10

## Libs
1. flask
2. flask_sqlalchemy

## DB
* sqlite

## Архитектура
* Похоже на распределенный монолит

## Минусы
1. Нельзя работать с новыми данными динамично. К примеру, я немогу отправить запрос на создание презентации, не переходя на новый урл, и потом отобразить уведомляшку. Или могу? Но нужен JS.
2. Есть возможность использовать только GET и POST запросы. Это не соответствует принципам REST API. HTML умеет только get, post. Для остальных типов нужно юзать js